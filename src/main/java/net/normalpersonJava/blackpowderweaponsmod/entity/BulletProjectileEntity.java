...
public class BulletProjectileEntity extends AbstractArrow implements ItemSupplier {
    public static final ItemStack PROJECTILE_ITEM = new ItemStack(ModItems.MUSKETBALL.get());
    ...
    public static BulletProjectileEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
        BulletProjectileEntity entityarrow = new BulletProjectileEntity(ModEntities.BULLET_PROJECTILE.get(), entity, world);
        ...
        world.playSound(null, entity.getX(), entity.getY(), entity.getZ(), Objects.requireNonNull(ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.generic.explode"))), SoundSource.PLAYERS, 1, 1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
        return entityarrow;
    }

    public static BulletProjectileEntity shoot(LivingEntity entity, LivingEntity target) {
        BulletProjectileEntity entityarrow = new BulletProjectileEntity(ModEntities.BULLET_PROJECTILE.get(), entity, entity.level());
        ...
        entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), Objects.requireNonNull(ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.generic.explode"))), SoundSource.PLAYERS, 1, 1f / (RandomSource.create().nextFloat() * 0.5f + 1));
        return entityarrow;
    }
